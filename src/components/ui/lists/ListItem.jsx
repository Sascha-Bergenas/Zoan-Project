...
export default function ListItem({session, evenOdd}) {
    const {started_at, title, category, ended_at, comment, mood} = session
    ...
    return(
        <li className={`${styles.listItem} ${evenOdd === "even" ? styles.even : styles.odd}`}>
            {/* <span>{date}</span> */}
            <span className={styles.cutLine}>{title}<span></span></span>
            <span>{category}</span>
            <span>{startTime}</span>
            <span>{stopTime}</span>
            {/* <span>{zoanTime}</span> */}
            <span className={styles.cutLine}>{comment}<span></span></span>
